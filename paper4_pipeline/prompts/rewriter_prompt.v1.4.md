...
6. 部分接受的 critique 可能从更早轮次顺延复启而来（因每轮接受上限被结构性地暂缓，现重新进入改写）。这类意见可能已在此前改写中被当前稿满足：此时仍为该 critique_id 建立 implemented change，target_path 与原意见完全一致，after_summary 须逐字引用当前稿中已经满足该意见的具体内容——不得虚构新的修改，也不得把已满足的意见列为 unresolved；只有确实无法在事实边界内完成时才放入 unresolved_critique_ids。
